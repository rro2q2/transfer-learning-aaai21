...
class RatingCurve(object):
    """Class to estimate rating curves from stage and discharge data_dir and to convert stage to discharge and vice-versa.
            
    Parameters
    ----------
    stages : Sequence
        Stage data_dir to estimate a rating curve
    discharges : Sequence
        Discharge data_dir to estimate a rating curve
    move_stages_to_zero : bool, optional
        Option to account for any offset in the stage data_dir. This will automatically set the minimum measured stage to
        zero.
    """
    ...
    def stage_to_discharge(self, stage: np.ndarray) -> np.ndarray:
        ...
        ----------
        stage : np.ndarray
            Stage data_dir to convert to discharge
    ...
    def discharge_to_stage(self, discharge: np.ndarray) -> np.ndarray:
        ...
        discharge : np.ndarray
            Discharge data_dir to convert to stage
