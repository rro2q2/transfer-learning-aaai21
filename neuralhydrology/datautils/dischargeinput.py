...
def shift_discharge(data_dir: Path, basins: List[str], dataset: str, shift: int = 1,
    ...
    """Return shifted discharge data_dir.
    
    This function returns the shifted discharge data_dir for each basin in `basins`. Useful when training
    ...
    data_dir available as input feature.
    ...
    for basin in tqdm(basins, file=sys.stdout):
        ...
        # shift discharge data_dir by `shift` time steps
        df[f"{discharge_col}_t-{shift}"] = df[discharge_col].shift(shift)

        # remove all columns from data_dir set except shifted discharge
