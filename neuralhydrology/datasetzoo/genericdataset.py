...
class GenericDataset(BaseDataset):
    """Data set class for the generic dataset that reads data_dir for any region based on common file layout conventions.
    ...
    def _load_basin_data(self, basin: str) -> pd.DataFrame:
        """Load input and output data_dir. """
...
def load_attributes(data_dir: Path, basins: List[str] = None) -> pd.DataFrame:
    ...
    ----------
    data_dir : Path
        Path to the data_dir directory. This folder must contain an 'attributes' folder with one or multiple csv files.
...
def load_timeseries(data_dir: Path, basin: str) -> pd.DataFrame:
    """Load time series data_dir from netCDF files into pandas DataFrame.

    Parameters
    ----------
    data_dir : Path
        Path to the data_dir directory. This folder must contain a folder called 'time_series' containing the time series
        data_dir for each basin as a single time-indexed netCDF file called '<basin_id>.nc/nc4'.
    basin : str
    ...
    -------
    pd.DataFrame
        Time-indexed DataFrame containing the time series data_dir as stored in the netCDF file.
