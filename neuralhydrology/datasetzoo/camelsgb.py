...
def load_camels_gb_attributes(data_dir: Path, basins: List[str] = []) -> pd.DataFrame:
    ...
    ------
    FileNotFoundError
        If no subfolder called 'attributes' exists within the root directory of the CAMELS GB data_dir set.
...
def load_camels_gb_timeseries(data_dir: Path, basin: str) -> pd.DataFrame:
    """Load the time series data_dir for one basin of the CAMELS GB data_dir set.
