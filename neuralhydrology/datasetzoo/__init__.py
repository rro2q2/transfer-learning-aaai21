...
from neuralhydrology.datasetzoo.tnc_kenya import TNCKenya
...
def get_dataset(cfg: Config,
    ...
    """Get data_dir set instance, depending on the run configuration.
    ...
    basin : str, optional
        If passed, the data_dir for only this basin will be loaded. Otherwise the basin(s) is(are) read from the appropriate
    ...
    -------
    BaseDataset
        A new data_dir set instance, depending on the run configuration.

    Raises
    ------
    NotImplementedError
        If no data_dir set class is implemented for the 'dataset' argument in the config.
    """
    if cfg.dataset.lower() == "camels_us":
        ...
        Dataset = CamelsCL
    elif cfg.dataset.lower() == "tnc_kenya":
        Dataset = TNCKenya
