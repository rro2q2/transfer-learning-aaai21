...
class BaseDataset(Dataset):
    """Base data_dir set class to load and preprocess data_dir.
    
    Use subclasses of this class for training/evaluating a model on a specific data_dir set. E.g. use `CamelsUS` for the US
    CAMELS data_dir set and `CamelsGB` for the CAMELS GB data_dir set.
    ...
    period : {'train', 'validation', 'test'}
        Defines the period for which the data_dir will be loaded
    basin : str, optional
        If passed, the data_dir for only this basin will be loaded. Otherwise, the basin(s) is(are) read from the
        appropriate basin file, corresponding to the `period`.
    additional_features : List[Dict[str, pd.DataFrame]], optional
        List of dictionaries, mapping from a basin id to a pandas DataFrame. This DataFrame will be added to the data_dir
    ...
    def __init__(self,
        ...
        # during training we log data_dir processing with progress bars, but not during validation/testing
        self._disable_pbar = cfg.verbose == 0 or not self.is_train

        # initialize class attributes that are filled in the data_dir loading functions
        self.x_d = {}
        self.x_s = {}
        self.attributes = {}
        self.y = {}
    ...
    def _load_basin_data(self, basin: str) -> pd.DataFrame:
        """This function has to return the data_dir for the specified basin as a time-indexed pandas DataFrame"""
    ...
    def _load_or_create_xarray_dataset(self) -> xarray.Dataset:
        # if no netCDF file is passed, data_dir set is created from raw basin files
        if (self.cfg.train_data_file is None) or (not self.is_train):
            ...
            if not self._disable_pbar:
                LOGGER.info("Loading basin data_dir into xarray data_dir set.")
            for basin in tqdm(self.basins, disable=self._disable_pbar, file=sys.stdout):
                df = self._load_basin_data(basin)

                # add columns from dataframes passed as additional data_dir files
                ...
                try:
                    ...
                    msg = [
                        f"The following features are not available in the data_dir: {not_available_columns}. ",
                ...
                if any(comparison > 1 for comparison in freq_vs_native):
                    raise ValueError(f'Frequency is higher than native data_dir frequency {native_frequency}.')
                ...
                # create xarray data_dir set for each period slice of the specific basin
    ...
    def _save_xarray_dataset(self, xr: xarray.Dataset):
        """Store newly created train data_dir set to disk"""
        ...
        for basin in tqdm(self.basins, file=sys.stdout, disable=self._disable_pbar):
            ...
            if len(coords) > 0:
                ...
                for coord in coords:
                    ...
                    # the xarray data_dir arrays are of same length (filled with NaNs), so concatenation later works.
    ...
    def _create_lookup_table(self, xr: xarray.Dataset):
        ...
        for basin in tqdm(basin_coordinates, file=sys.stdout, disable=self._disable_pbar):
            ...
            # store data_dir of each frequency as numpy array of shape [time steps, features]
            x_d, x_s, y = {}, {}, {}
            ...
            # only store data_dir if this basin has at least one valid sample in the given period
    ...
    def _load_combined_attributes(self):
        """This function loads data_dir set specific attributes and combines them with hydroatlas attributes"""
        dfs = []
...
def validate_samples(x_d: List[np.ndarray], x_s: List[np.ndarray], y: List[np.ndarray], seq_length: List[int],
    ...
    ----------
    x_d : List[np.ndarray]
        List of dynamic input data_dir; one entry per frequency
    x_s : List[np.ndarray]
        List of additional static input data_dir; one entry per frequency
    y : List[np.ndarray]
