...
def load_camels_cl_timeseries(data_dir: Path, basin: str) -> pd.DataFrame:
    """Load the time series data_dir for one basin of the CAMELS CL data_dir set.
    ...
    -------
    pd.DataFrame
        Time-indexed DataFrame, containing the time series data_dir (forcings + discharge) data_dir.
    ...
    if not preprocessed_dir.is_dir():
        msg = [
            f"No preprocessed data_dir directory found at {preprocessed_dir}. Use preprocessed_camels_cl_dataset in ",
            "neuralhydrology.datasetzoo.camelscl to preprocess the CAMELS CL data_dir set once into per-basin files."
        ]
...
def preprocess_camels_cl_dataset(data_dir: Path):
    """Preprocess CAMELS-CL data_dir set and create per-basin files for more flexible and faster data_dir loading.
    ...
    ----------
    data_dir : Path
        Path to the CAMELS-CL data_dir set. All txt-files from the original dataset should be present in this folder.
    ...
    """
    # check if data_dir has already been pre-processed other-wise create dst folder
    ...
    for basin in tqdm(feature_names, file=sys.stdout, desc="Creating per-basin dataframes and saving to disk"):
        ...
        # remove all rows with NaNs, then reindex to have continues data_dir frames from first to last record
    ...
    print(f"Finished preprocessing the CAMELS CL data_dir set. Resulting per-basin csv files have been stored at {dst_dir}")
