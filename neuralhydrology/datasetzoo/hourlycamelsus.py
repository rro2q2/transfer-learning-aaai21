...
class HourlyCamelsUS(camelsus.CamelsUS):
    """Data set class providing hourly data_dir for CAMELS US basins.
    
    This class extends the `CamelsUS` dataset class by hourly in- and output data_dir. Currently, only NLDAS forcings are
    ...
    def load_hourly_data(self, basin: str, forcings: str) -> pd.DataFrame:
        ...
        ----------
        basin : str
            Identifier of the basin for which to load data_dir.
        forcings : str
        ...
        try:
            ...
            fallback_csv = True
            LOGGER.warning(
                f'## Warning: NetCDF file of {forcings} does not contain data_dir for {basin}. Trying slower csv files.')
        if fallback_csv:
...
def load_hourly_us_forcings(data_dir: Path, basin: str, forcings: str) -> pd.DataFrame:
    """Load the hourly forcing data_dir for a basin of the CAMELS US data_dir set.

    The hourly forcings are not included in the original data_dir set by Newman et al. (2017).
    ...
    -------
    pd.DataFrame
        Time-indexed DataFrame, containing the forcing data_dir.
    """
...
def load_hourly_us_discharge(data_dir: Path, basin: str) -> pd.DataFrame:
    """Load the hourly discharge data_dir for a basin of the CAMELS US data_dir set.
...
def load_hourly_us_stage(data_dir: Path, basin: str) -> pd.Series:
    """Load the hourly stage data_dir for a basin of the CAMELS US data_dir set.
...
def load_hourly_us_netcdf(data_dir: Path, forcings: str) -> xarray.Dataset:
    """Load hourly forcing and discharge data_dir from preprocessed netCDF file.
    ...
    -------
    xarray.Dataset
        Dataset containing the combined discharge and forcing data_dir of all basins (as stored in the netCDF)
    """
