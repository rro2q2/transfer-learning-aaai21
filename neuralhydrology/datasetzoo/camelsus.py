...
class CamelsUS(BaseDataset):
    """Data set class for the CAMELS US data_dir set by [#]_ and [#]_.
    ...
    .. [#] Addor, N., Newman, A. J., Mizukami, N. and Clark, M. P.: The CAMELS data_dir set: catchment attributes and
...
def load_camels_us_attributes(data_dir: Path, basins: List[str] = []) -> pd.DataFrame:
    ...
    data_dir : Path
        ...
        data_dir set) containing the corresponding txt files for each attribute group.
...
def load_camels_us_forcings(data_dir: Path, basin: str, forcings: str) -> Tuple[pd.DataFrame, int]:
    """Load the forcing data_dir for a basin of the CAMELS US data_dir set.
    ...
    data_dir : Path
        ...
        the original CAMELS data_dir set. In each HUC folder are the forcing files (.txt), starting with the 8-digit basin
        id.
    basin : str
...
def load_camels_us_discharge(data_dir: Path, basin: str, area: int) -> pd.Series:
    ...
    data_dir : Path
        ...
        subdirectories (for the 18 HUCS) as in the original CAMELS data_dir set. In each HUC folder are the discharge files
